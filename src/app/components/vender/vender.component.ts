...
export class VenderComponent implements OnInit {
              ...
              private toast: ToastrService) {
                this.router.params.subscribe((resp) => {
                  ...
                  this.recepcion.obtenerRecepcion(resp.id).subscribe((resp: any) => {
                    ...
                    this.montoActualizado = parseInt(resp.recepcion[0].precio);
                  });
                });
                this.getProductos();
              }
  ...
  volver() {
    this.route.navigate(['/habitaciones-ocupadas']);
  }
  // tslint:disable-next-line: typedef
  getProductos() {
...
getTotal(): void {
 ...
 for (let i = 0; i < this.arrayProducto.length; i++) {
  ...
  resultado = resultado + (parseInt(this.arrayProducto[i].precio) * this.arrayProducto[i].cantidad);
 }
  ...
  if (this.arrayProducto.length === 0){
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.montoActualizado += this.calcularTotal;
        this.mostrar = 2;
        const datoEnviar = {
          id: this.recepcionObtenida.id,
          habitacion_id: this.recepcionObtenida.habitacion_id,
